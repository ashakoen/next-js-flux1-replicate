...
export default function Component() {
	...
	if (name === 'privateLoraName') {
	  ...
	  setFormData((prev) => {
		const updatedFormData = {
		  ...prev,
		  privateLoraName: value // Correctly set the value
		};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
	...
	if (isGenerating) {
	  return; // Prevent double submissions if already generating
	}

	isPolling.current = true;
	abortControllerRef.current = new AbortController(); // Initialize once

	const [loraName, loraVersion] = formData.privateLoraName.split(':');

	//const loraName = formData.privateLoraName.trim();
	//const loraVersion = formData.privateLoraVersion.trim();

  // Log loraName and loraVersion to the console
  console.log('LoRA Name:', loraName);
  console.log('LoRA Version:', loraVersion);

	let replicateParams;
	if (loraName && loraVersion) {
	  // Private LoRA provided
	  replicateParams = {
		version: loraVersion,
		input: {
	  ...
	  replicateParams = {
		...
		model: formData.model, // Include model in payload for backend to decide URL
	  };
	}

	console.log('Replicate Params:', replicateParams); // Debugging log
	...
	try {
	  ...
	  pollForResult(getUrl); // Call poll loop
  ...
  const pollForResult = async (url: string) => {
	if (!isPolling.current) return; // Only proceed if polling is active
  ...
  const validateLoraModel = (modelName: string) => {
	...
	// The logic for actual model validation should be implemented here
  ...
  const handleCancel = async () => {
	...
	try {
	  if (abortController.current) {
		abortController.current.abort(); // Abort the fetch requests
	  }
					  ...
					  <div className="space-y-4">
						<div>
						  ...
						  <Textarea
							...
							onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => handleInputChange(e as unknown as React.ChangeEvent<HTMLInputElement>)}
